...
import svgPaths from "../imports/svg-1x9yza2bc6";
import imgGoogle1 from "figma:asset/8e4241399baefbe8f8feffab0fe67682e140e1b1.png";

function CreateAccountButton({ onClick, isPressed }: { onClick: () => void; isPressed: boolean }) {
  return (
    <motion.div
      className="absolute bg-[#0088ff] box-border content-stretch flex gap-[10px] h-[55px] items-center justify-center left-[39px] px-[109px] py-[21px] rounded-[4px] top-[511px] w-[317px] cursor-pointer"
      onClick={onClick}
...
function CreateAnAccount({ onLogin }: { onLogin: () => void }) {
  return (
...
function Group8({ onLogin }: { onLogin: () => void }) {
  return (
    <div className="absolute contents left-1/2 top-[591px] translate-x-[-50%]">
      <CreateAnAccount onLogin={onLogin} />
      <GoodleFbSignIn />
    </div>
  );
}
...
function Eye({ onClick, showPassword }: { onClick: () => void; showPassword: boolean }) {
  return (
    <motion.div
      className="absolute left-[317px] size-[20px] top-[393px] cursor-pointer"
      data-name="eye"
      onClick={onClick}
...
function Password({
  ...
  return (
    <div className="absolute contents left-[36px] top-[375px]" data-name="Password">
      <motion.div
        className="absolute bg-[#f3f3f3] border border-solid h-[55px] left-[36px] rounded-[10px] top-[375px] w-[317px]"
      ...
      <input
        ...
        className="absolute bg-transparent border-0 font-['Montserrat:Medium',sans-serif] font-medium h-[55px] left-[105px] outline-none text-[#676767] text-[12px] top-[375px] w-[200px]"
      />
      {isFocused && !value && (
        <motion.div
          className="absolute bg-[#0088ff] h-[20px] left-[107px] top-[393px] w-[1px]"
          animate={{ opacity: [1, 0] }}
...
function Eye1({ onClick, showPassword }: { onClick: () => void; showPassword: boolean }) {
  return (
    <motion.div
      className="absolute left-[318px] size-[20px] top-[320px] cursor-pointer"
      data-name="eye"
      onClick={onClick}
      whileHover={{ scale: 1.1 }}
      whileTap={{ scale: 0.9 }}
    >
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 20 20">
        <g clipPath="url(#clip0_1_1842)" id="eye">
          <path d={svgPaths.p3d74ed00} id="Vector" stroke={showPassword ? "#0088ff" : "#626262"} strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" />
          <path d={svgPaths.p3b27f100} id="Vector_2" stroke={showPassword ? "#0088ff" : "#626262"} strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" />
          {!showPassword && (
            <line x1="2" y1="2" x2="18" y2="18" stroke="#626262" strokeWidth="2" strokeLinecap="round" />
...
function Password1({
  ...
  return (
    <div className="absolute contents left-[37px] top-[302px]" data-name="Password">
      <motion.div
        className="absolute bg-[#f3f3f3] border border-solid h-[55px] left-[37px] rounded-[10px] top-[302px] w-[317px]"
        animate={{ borderColor: isFocused ? "#0088ff" : "#a8a8a9" }}
        transition={{ duration: 0.2 }}
      />
      <Group6 />
      <input
        ...
        className="absolute bg-transparent border-0 font-['Montserrat:Medium',sans-serif] font-medium h-[55px] left-[82px] outline-none text-[#676767] text-[12px] top-[302px] w-[230px]"
      />
      {isFocused && !value && (
        <motion.div
          className="absolute bg-[#0088ff] h-[20px] left-[84px] top-[320px] w-[1px]"
          animate={{ opacity: [1, 0] }}
          transition={{ duration: 0.8, repeat: Infinity }}
        />
      )}
      <Eye1 onClick={onTogglePassword} showPassword={showPassword} />
    </div>
  );
}
...
function UsernameOrEmail({
  ...
  return (
    <div className="absolute contents left-[37px] top-[229px]" data-name="Username or email">
      <motion.div
        className="absolute bg-[#f3f3f3] border border-solid h-[55px] left-[37px] rounded-[10px] top-[229px] w-[317px]"
        animate={{ borderColor: isFocused ? "#0088ff" : "#a8a8a9" }}
        transition={{ duration: 0.2 }}
      />
      <input
        ...
        className="absolute bg-transparent border-0 font-['Montserrat:Medium',sans-serif] font-medium h-[55px] left-[106px] outline-none text-[#676767] text-[12px] top-[229px] w-[230px]"
      />
      {isFocused && !value && (
        <motion.div
          className="absolute bg-[#0088ff] h-[20px] left-[108px] top-[247px] w-[1px]"
          animate={{ opacity: [1, 0] }}
          transition={{ duration: 0.8, repeat: Infinity }}
        />
      )}
...
function BackButton({ onClick }: { onClick: () => void }) {
  return (
    <motion.div
      className="absolute h-[19px] left-[37px] top-[71px] w-[9.5px] cursor-pointer"
      onClick={onClick}
...
export default function SignUp({
  ...
  return (
    <div className="bg-white relative size-full" data-name="Sign Up">
      ...
      <CreateAccountButton onClick={handleCreate} isPressed={createPressed} />
      <Group8 onLogin={onLogin} />
